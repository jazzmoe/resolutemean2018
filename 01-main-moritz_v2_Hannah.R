...
DUR.LOC.OCT2 <- DUR.LOC.OCT 

DUR.LOC.OCT2$location <- factor(DUR.LOC.OCT$location, levels=c("Russian Federation", "China", "United States",
                                                         "Netherlands","Brazil"))
...
plot1 <- DUR.LOC.OCT2 %>%
  ...
  geom_area()
plot1 + labs(x = "Calendar Weeks of Month October", 
             y = "Number of Detected IP Connections") +
  ggtitle("Share of Detected IP Connections by Country") +
  guides(fill=guide_legend(title="Top 5 Cyber-Space Powers"))+
  scale_fill_manual(values=c("tomato3", "olivedrab1", "yellow2", 'purple', 'deepskyblue'))
...
plot3 <- DUR.LOC.DEC %>% ggplot(aes(x = week, y = req.per.month, fill = location)) + 
  geom_area()
plot3 + labs(x = "Calendar Weeks of Month DECEMBER", 
             y = "Number of Detected IP Requests") +
  ggtitle("Share of Detected IPs by Country") +
  guides(fill=guide_legend(title="Top 5 Cyber-Space Powers"))+
  scale_fill_manual(values=c("purple", "olivedrab1", "yellow2", 'tomato3', 'deepskyblue'))

# April 2015
DUR.LOC.APRIL <- DUR.LOC %>% filter(month == "Apr") %>% group_by(location, week) %>%
  summarize(req.per.month = n())

DUR.LOC.APRIL2 <- DUR.LOC.APRIL

DUR.LOC.APRIL2$location <- factor(DUR.LOC.APRIL$location, levels=c("Russian Federation", "China", "United States",
                                                               "Netherlands","Brazil"))


plot2 <- DUR.LOC.APRIL2 %>% ggplot(aes(x = week, y = req.per.month, fill = location)) + 
  geom_area()
plot2 + labs(x = "Calendar Weeks of Month April", 
             y = "Number of Detected IP Connections") +
  ggtitle("Share of Detected IP Connections by Country") +
  guides(fill=guide_legend(title="Top 5 Cyber-Space Powers"))+
  scale_fill_manual(values=c("tomato3", "olivedrab1", "yellow2", 'purple', 'deepskyblue'))
...
plot4 <- DUR.LOC.OVERALL %>% ggplot(aes(x = week, y = req.per.month, fill = location)) + 
  geom_area()
plot4 + labs(x = "Calendar Weeks of the Experiment", 
             y = "Number of Detected IP Requests") +
  ggtitle("Share of Detected IPs by Country") +
  guides(fill=guide_legend(title="Top 5 Cyber-Space Powers"))+
  scale_fill_manual(values=c("purple", "olivedrab1", "yellow2", 'tomato3', 'deepskyblue'))
...
plot5 <- DUR.LOC.2014 %>% 
           ggplot(aes(x = week, y = req.per.month, fill = location)) + 
           geom_area()
plot5 + labs(x = "Calendar Weeks of the Experiment",
             y = "Number of Detected IP Requests") +
  ggtitle("Share of Detected IPs by Country") +
  guides(fill=guide_legend(title="Top 5 Cyber-Space Powers"))+
  scale_fill_manual(values=c("purple", "olivedrab1", "yellow2", 'tomato3', 'deepskyblue'))
...
# comments
#I can't get the code for the heatmap to run properly, the 